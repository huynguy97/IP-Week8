...
const TrackDisplay ShowAll = {true, true, true, true, true, true, true, true};
const TrackDisplay ShowArtistOnly = {true, false, false, false, false, false, false, false};
const TrackDisplay ShowArtistAndCd = {true, true, false, false, false, false, false, false};
...
*/
    ...
    r = (a.seconds + b.seconds)%60;
    m = (a.seconds + b.seconds)/60;

    len.minutes = a.minutes + b.minutes + m;
    len.seconds = r;
...
*/
    ...
    if(lt.showTrack){
        cout << "Track = " << track.track << "\t";
    }
    if(lt.showYear){
        cout << "Year = " << track.year << "\t";
    }
    cout << endl;
}
...
bool isStringInVector(vector<string>& source, string str)
{
    //             
    assert(true);
    /*                
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    */
    int counter = 0;
    vector<string> artistsFound;
    for(int i=0; i <  static_cast<int>(tracks.size()); i++){
        if (match(artist, tracks[i].artist)){
             if(!isStringInVector(artistsFound, tracks[i].artist)){
                artistsFound.push_back(tracks[i].artist);
                counter++;
                if(display){
                show_track(tracks[i], ShowArtistOnly);
                }
             }
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    */
    int counter = 0;
    vector<string>CdFound;
    for(int i=0; i < static_cast<int>(tracks.size()); i++){
        if (match(artist, tracks[i].artist)){
             if(!isStringInVector(CdFound, tracks[i].cd)){
                CdFound.push_back(tracks[i].cd);
                counter++;
                if(display){
                show_track(tracks[i], ShowArtistAndCd);
                }
             }
...
*/
    string c;
    getline(in, c); track.artist = c;
    getline(in, c); track.cd = c;
    in >> track.year; in.get();
    in >> track.track; in.get();
    getline(in, c); track.title = c;
    getline(in, c); track.tags = c;
    in >> track.time; in.get();
    getline(in, c); track.country = c;
    getline(in, c);
    return in;
}

int readFile(vector<Track>& DB, string filename)
{
    ifstream inputfile;
...
*/
    //                                      
    //                              
    cout << readFile(TracksDB, "Tracks.txt") << " read in." << endl;
