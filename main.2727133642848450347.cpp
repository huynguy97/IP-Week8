...
*/
    int seconds = length.seconds % 60;
    int minutes = length.minutes / 60;
...
*/
    Length sum_length;
    int sum_seconds = a.seconds + b.seconds;
    int sum_minutes = a.minutes + b.minutes;

    sum_length.seconds = sum_seconds % 60;
    sum_length.minutes = sum_minutes + sum_seconds / 60;
...
*/
    ...
    if (display)
        cout << "Found total of " << cc << " matches.\n";
    return cc;
}
...
*/
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        {
            bool alreadyFound = false;
            for (int ii = 0; ii < cds.size(); ii++)
            {
                if (cds[ii] == tracks[i].cd)
                    alreadyFound = true;
            }
            if (alreadyFound)
                continue;
...
*/
    ...
    {
        cout << "File \"" << filename << "\" was not opened correctly.";
        return -1;
    }
    ...
    while(!input_file.eof())
    {
        ...
        input_file >> temp;
        input_file.ignore();
        if (temp.artist.empty()) //                                    
            continue;
        show_track(temp, td);
        tracksDB.push_back(temp);
        track_count++;
    }
    cout << "Total read entries: " << track_count << endl;
