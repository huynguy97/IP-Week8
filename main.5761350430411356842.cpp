...
string found_artists[1000];
string found_cds[1000];
string cds[1000];
...
*/
    char z;
    in >> length.minutes >> z >> length.seconds;
    return in;
}
...
*/
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(track, tracks[i].title))
        {
            ...
            if (display)
            {
                TrackDisplay bla = {true, true, true, true, true, true, true, true};
                Track yes = {   tracks[i].artist,
                                tracks[i].cd,
                                tracks[i].year,
                                tracks[i].track,
                                tracks[i].title,
...
*/
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(artist, tracks[i].artist))
        {
            ...
            {
            if (tracks[i].artist == found_artists[k])
            {
                uni = false;
                e++;
            }
            ...
            if (display)
            {
                TrackDisplay bla = {true, false, false, false, false, false, false, false};
                Track yes = {   tracks[i].artist,
                                tracks[i].cd,
                                tracks[i].year,
                                tracks[i].track,
                                ...
                                tracks[i].country };
                show_track(yes, bla);
                found_artists[i-e] = tracks[i].artist;
            }
            }
        }
    }
...
int match_cds (vector <Track>& tracks, string artist, bool display)
{
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(artist, tracks[i].artist))
        {
            ...
            if (display)
            {
                TrackDisplay bla = {true, true, true, false, false, false, false, false};
                Track yes = {   tracks[i].artist,
                                tracks[i].cd,
                                tracks[i].year,
                                tracks[i].track,
...
*/
    ...
    else
    {
        ...
        cout << "Number of entries read: " << counter-1 << endl;
    }
	return 0;
}
