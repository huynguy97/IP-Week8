...
*/
    ...
    else if (length.seconds == 0)
        out << length.minutes << ":00";
    else
    ...
    for (int i = 0; i < tracks.size(); i++)
        if (match(artist, tracks[i].artist))
        {
            ...
            for (int j=0; j<ma.size(); j++)
                if (tracks[i].artist == ma[j])
                    check = 0;
            if (check)
            {
                counter++;
                ma.push_back(tracks[i].artist);
                if (display)
...
//                
    ...
    for (int i = 0; i < tracks.size(); i++)
        ...
        {
            bool check = 1;
            for (int j=0; j<mcd.size(); j++)
                if (tracks[i].cd == mcd[j])
                    check = 0;
            if (check)
            {
                counter++;
                mcd.push_back(tracks[i].cd);
                if (display)
...
*/
    ...
    while (infile)
    {
        infile >> single_track_info;
        tracklist.push_back(single_track_info);
        counter++;
    }
    for (int i=0; i<tracklist.size(); i++)
        show_track(tracklist[i],{1,1,1,1,1,1,1,1});
    infile.close();
	return 0;
}
