...
*/
    ...
    for (int i =0; (unsigned)i<tracks.size(); i++)
    {
        if(match(track, tracks[i].title) )
        {
            matches ++;
            if(display)
                show_track(tracks[i], show_all);
        }
    }
...
*/
    TrackDisplay show_artist = {true, false, false, false, false, false, false, false};
    int matches=0;
    string last_artist;

    for (int i =0; (unsigned)i<tracks.size(); i++)
    {
        if(match(artist, tracks[i].artist) && tracks[i].artist != last_artist)
        {
            matches ++;
            last_artist = tracks[i].artist;
            if(display)
                show_track(tracks[i], show_artist);
        }
    }
...
*/
    TrackDisplay show_cd = {true, true, true, false, false, false, false, false};
    int matches=0;
    string last_cd, last_artist;

    for (int i =0; (unsigned)i<tracks.size(); i++)
    {
        if(match(artist, tracks[i].artist) && (tracks[i].cd != last_cd ||
            tracks[i].artist != last_artist))
        {
            matches ++;
            last_cd = tracks[i].cd;
            last_artist = tracks[i].artist;
            if(display)
                show_track(tracks[i], show_cd);
        }
    }
...
*/
    ...
    for (int i =0; (unsigned)i<tracks.size(); i++)
    {
        if((tracks[i].cd != last_cd || tracks[i].artist != last_artist))
        {
            matches ++;
            last_cd = tracks[i].cd;
            last_artist = tracks[i].artist;
...
*/
    ...
    cout << "Number of entries: " << match_tracks(database,"", false) << endl;
    infile.close();
