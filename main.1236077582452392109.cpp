...
int match_tracks (vector<Track>& tracks, string track, bool display){
    ...
    cin >> sub;
    cout << "do you want to display the track information? (y/n)\n";
    cin >> choice;
    if (choice == "y")
        display = true;
    else display = false;
    for (int i = 0 ; i < 5981; i++){
        test = tracks[i];
        track = test.title;
        if (match(sub, track)){
            counter++;
    ...
    cout << "There are " << counter << " tracks with " <<  sub << " in the title " << '\n';
    return counter;
}
...
int match_artists (vector<Track>& tracks, string track, bool display){
    ...
    cout << "There are " << counter << " artists with " <<  sub << " in is/her name " << '\n';
    return counter;
}
...
int match_cds (vector<Track>& tracks, string track, bool display){
    ...
    cout << "There are " << counter << " cds with " <<  sub << " in the cd title " << '\n';
    return counter;
}
...
int number_of_cds (vector<Track>& tracks){
    ...
    cout << "there are " << counter << " CDs in the file\n";
    return counter;
}
...
void read_file (ifstream& in, vector<Track>& tracks){
    //             
    assert(true);
    //              
    /*                                                           
    ...
    Track track;
    string filename;
    cout << "enter the filename you want to open and press enter\n";
    cin >> filename;
    in.open(filename);
    if (in.is_open()){
        ...
        for (int i = 0; i < no_tracks-1; i++){
            show_track(tracks[i], lt);
        }
        cout << "\n" << no_tracks - 1 << " tracks are in the file\n";
    }
    if (!in.is_open()){
...
*/
    ...
    match_tracks(tracks,track, display);
    match_artists(tracks,track, display);
    match_cds(tracks,track, display);
