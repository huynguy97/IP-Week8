...
*/
		...
		if (sum.seconds >= 60)
		{
			...
			j = sum.seconds%60 ;
			sum.seconds = j ;
			sum.minutes = sum.minutes + (i-(j/60)) ;
		}
		return sum ;
}
...
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    {
        if (match(track, testDB[i].title))
        {
            count++;
            if (display)
                show_track(testDB[i],lt);
        }
    }
    cout << "The number of matches found is: " << count << endl;
    return count;
}
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    vector<string> countartist;
    bool occurs = false;
    int count=0;
    for (int i=0; i<testDB.size(); i++)
    {
        if (match(artist, testDB[i].artist))
        {
            for (int j=0; j<countartist.size(); j++)
            {
                if (match(testDB[i].artist, countartist[j]))
                    occurs = true;
            }
            if (!occurs)
            {
                countartist.push_back(testDB[i].artist);
                count++;
                if (display)
                    show_track(testDB[i], lt);
            }
    ...
    cout << "The number of matched artists found is: " << count << endl;
    return count;
}
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    vector<string> countartist;
    vector<string> countcd;
    bool artist_occurs = false;
    bool cd_occurs = false;
    int count=0;
    for (int i=0; i<testDB.size(); i++)
    {
        if (match(artist, testDB[i].artist))
        {
            ...
            if (!artist_occurs)
            {
                for (int k=0; k<countcd.size(); k++)
                {
                    if (match(testDB[i].cd, countcd[k]))
                    cd_occurs = true;
                }
                if (!artist_occurs && !cd_occurs)
                {
                    countcd.push_back(testDB[i].cd);
                    count++;
                    if (display)
                        show_track(testDB[i], lt);
                }
    ...
    cout << "The number of cd's that matched with artists found is: " << count << endl;
    return count;
}
...
int number_of_cds (vector<Track>& tracks)
{
    ...
    for (int i=0; i<testDB.size(); i++)
    {
        for (int j=0; j<countcd.size(); j++)
        {
            if (match(countcd[j], testDB[i].cd))
                occurs = true;
        }
        if (!occurs)
        {
    ...
    cout << "The number of albums in the database is: " << count << endl;
}
...
*/
    string track = "el" ;
    match_tracks(testDB, track, true);
    string artist = "el" ;
    match_artists(testDB, artist, true);
    match_cds(testDB, artist, true);
