...
*/
    char rest;
    in >> length.minutes >> rest >> length.seconds;
    return in;
}
...
*/
    int match_counter = 0;
    vector<string> matching_artists;
    for(int i = 0; i < tracks.size(); i++)
    {
        bool new_artist = true;

        for(int j = 0; j < matching_artists.size(); j++)
        {
            if(matching_artists[j] == tracks[i].artist)
            {
                new_artist = false;
            }
        }
        ...
        if(new_artist)
        {
            ...
            {
                match_counter++;
                matching_artists.push_back(tracks[i].artist);
                if(display)
                {
...
*/
    int match_counter = 0;
    vector<string> matching_cds;
    for(int i = 0; i < tracks.size(); i++)
    {
        bool new_cd = true;

        for(int j = 0; j < matching_cds.size(); j++)
        {
            if(matching_cds[j] == tracks[i].cd)
            {
                new_cd = false;
            }
        }
        ...
        if(new_cd)
        {
            ...
            {
                match_counter++;
                matching_cds.push_back(tracks[i].cd);
                if(display)
                {
...
*/
    ...
    input_file >> storage_track;
    while(input_file)
    {
        tracksDB.push_back(storage_track);
        input_file >> storage_track;
        show_track(tracksDB.back(), {true, true, true, true, true, true, true, true});
        number_of_tracks++;
    }
    cout << "The number of tracks is: " << number_of_tracks;
