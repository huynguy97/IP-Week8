...
*/
	string line;
	getline(in, line);
	length.minutes = length.seconds = 0;
	//             
	int x = 0;
	//                      
	//                                    
	while(x<static_cast<int>(line.length())&&!(line[x]>='0'&&line[x]<='9'))
		x++;
	//                         
	while(x<static_cast<int>(line.length())&&line[x]!=':'){
		length.minutes *= 10;
		length.minutes += line[x]-'0';
		x++;
	}
	x++;
	//                                
...
void removeEndingChars(string& s){
	//              
	assert(true);
	//               
	//                                                        
...
int match_artists(vector<Track>& tracks, string artist, bool display){
	...
	//                                                 
	vector<bool> check(static_cast<int>(tracks.size()),true);
	int count = 0;
	for (int i = 0;i<static_cast<int>(tracks.size());i++){
		if (check[i] and match(artist,tracks[i].artist)){
			count++;
			if (display){
				show_track(tracks[i],{true,false,false,false,false,false,false,false});
			}
			//                                  
			for (int j = i+1;j<static_cast<int>(tracks.size());j++){
...
int match_cds(vector<Track>& tracks, string artist, bool display){
	...
	for (int i = 0;i<static_cast<int>(tracks.size());i++){
		if (check[i] and match(artist,tracks[i].artist)){
			...
			//                               
			for (int j = i+1;j<static_cast<int>(tracks.size());j++){
				if (tracks[i].artist==tracks[j].artist and
					tracks[i].cd == tracks[j].cd){
					check[j] = false;
				}
			}
		}
...
istream& read_tracks(vector<Track>& res, istream& in){
	//              
	assert(true);
	//               
	//                                            
...
*/
	...
	//                       
	ifstream in;
	open_file(in, "Tracks.txt");
	vector<Track> tracks;
	read_tracks(tracks, in);
	//                 
	int returnval;
	cout << "Number of tracks: " << tracks.size() << endl;
	cout << "Output of match_tracks(tracks,\"el\",true):" << endl;
	returnval = match_tracks(tracks,"el",true);
	cout << "With return value: " << returnval << endl;
	cout << "Output of match_artists(tracks,\"el\",true):" << endl;
	returnval = match_artists(tracks,"el",true);
	cout << "With return value: " << returnval << endl;
	cout << "Output of match_cds(tracks,\"el\",true):" << endl;
	returnval = match_cds(tracks,"el",true);
	cout << "With return value: " << returnval << endl;
	cout << "Number of cd's: " << number_of_cds(tracks) << endl;
	return 0;
}
