...
*/
    int sumSec = a.seconds + b.seconds;
    int sumMin = 0;
    while (sumSec > 60)
    {
        sumSec = sumSec - 60;
    ...
    sumMin = sumMin + a.minutes + b.minutes;
    Length sum = {sumMin, sumSec};
    return sum;
}
...
*/
    ...
    if (lt.showTrack)
        cout << "Track number: " << track.track << endl;
    if (lt.showTitle)
...
*/
    ...
    for (int i = 0; i < numTracks; i++)
    {
        ...
        if (match(track, thisTrack.title))
        {
            if (display)
            {
                show_track(thisTrack, lt);
                cout << endl;
            }
            matches++;
        }
...
*/
    ...
    {
        Track thisTrack = tracks[i];
        if (match(artist, thisTrack.artist) && !(thisTrack.artist == lastArtist))
        {
            if (display)
            {
                lastArtist = thisTrack.artist;
                show_track(thisTrack, lt);
            }
            matches++;
        }
...
*/
    ...
    {
        Track thisTrack = tracks[i];
        if (match(artist, thisTrack.artist))
        {
            if (thisTrack.cd != lastAlbum)
            {
                if (display)
                {
                    show_track(thisTrack, lt);
...
void readFile (ifstream& musicFile, vector<Track>& readTracks)
{//              
    assert (musicFile.is_open());
/*                
...
*/
    ...
    while (musicFile)
    {
        ...
        show_track(track, lt);
        cout << endl;
        readTracks.push_back(track);
    }
    cout << endl << "Tracks read: " << numTracks << endl;
}
...
*/
    ...
    readFile(musicfile, readTracks);
    cout << "Tracks saved: " << readTracks.size();
    musicfile.close();
	return 0;
}
