...
*/
  return {
    a.minutes + b.minutes + ((a.seconds + b.seconds) / 60),
    (a.seconds + b.seconds) % 60
  };
}
...
int match_artists(vector<Track>&  tracks, string artist, bool display)  {
  ...
  for (int i = 0; i < size(tracks); i++) {
    if (match(tracks[i].artist,artist)) {
      ...
      int j = 0;
      while (j < size(artists) && !artistFound) {
        artistFound = artists[j] == tracks[i].artist;
        j++;
      }
      if (!artistFound) {
...
int number_of_cds(vector<Track>& tracks){
    ...
    for(int i = 0; i < size(tracks); i++){
        ...
        for(int j = 0; j < size(counted); j++){
            if(tracks[i].cd == counted[j]){
                should_count = false;
            }
        }
...
*/
  ...
  cout << endl << "Number of tracks: " << i << endl;
