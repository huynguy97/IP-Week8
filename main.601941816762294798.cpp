...
*/
    Length length;
    length.minutes = a.minutes+b.minutes;
    length.seconds = a.seconds+b.seconds;
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    if(display)
    {
        ...
        for(int i=0;i<vecsize;i++)
            if(tracks[i].artist != prev_artist )//                                                                                   
            {
...
//                                                                     
    ...
    cout << "The program has read " << tracks.size() << " entries.\n";
	return 0;
}
