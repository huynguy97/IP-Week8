...
*/
    ...
    for(int i = 0;i < tracks.size(); i++)
    {
        if(match(artist, tracks[i].artist))
        {
            ...
            if(display)
            {
                if(prevArtist != tracks[i].artist)
                {
                    show_track(tracks[i], matching);
                    prevArtist = tracks[i].artist;
                }
            }
        }
    }
...
void open_file(ifstream& infile, vector<Track>& DB)
{//              
    assert ( true );
/*                
...
*/
    ...
    while(infile)
    {
        ...
        getline(infile, temp.cd);
        infile >> temp.year;
        infile >> temp.track;
        infile.ignore(256, '\n');
        getline(infile, temp.title);
        getline(infile, temp.tags);
        infile >> temp.time;
        infile.ignore(256, '\n');
        getline(infile, temp.country);
        infile.ignore(256, '\n');
        DB.push_back(temp);
        i++;
    }
    cout << i-1 << " entries have been read" << endl;
}
