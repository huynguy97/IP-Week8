...
int match_artists (vector<Track>& tracks, string artist, bool display){
    ...
    */
    ...
    string lastMatch;
    for(size_t i = 0; i < tracks.size(); i++){
        if(match(artist, tracks[i].artist) && tracks[i].artist != lastMatch){
            lastMatch = tracks[i].artist;
            result++;
            if(display){
...
int match_cds (vector<Track>& tracks, string cd, bool display){
    //              
    assert(true);
    /*
    ...
    */
    ...
    string lastMatch;
    for(size_t i = 0; i < tracks.size(); i++){
        if(match(cd, tracks[i].cd) && tracks[i].cd != lastMatch){
            lastMatch = tracks[i].cd;
            result++;
            if(display){
...
*/
    ...
    if(!infile.is_open()){
        cout << "couldn't open file. Exiting...\n";
        return 0;
    }
    vector<Track> vec;
    Track t;
    ...
    cout << "Number of Entries read: " << vec.size() << endl;
