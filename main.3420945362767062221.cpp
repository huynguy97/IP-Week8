...
*/
    ...
    {
        current_track = tracks[i].title;
        if (match(track, current_track))
        {
            tracks_found++;
            if (display)
            {
...
*/
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        {
            if (current_artist == found_artists[j])
            {
                flag_repetition = false;
            }
        }

        if (match(artist, current_artist) && flag_repetition)
        {
            found_artists.push_back(current_artist);
            artists_found++;
            if (display)
            {
...
*/
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        if (match(artist, current_artist) && flag_repetition)
        {
            found_cds.push_back(current_cd);
            cds_found++;
            if (display)
            {
...
*/
    ...
    {
         infile >> current_track;
         tracks.push_back(current_track);
         show_track(current_track, td);
         i++;
    }
    cout << "\nnumber of entries: " << i << "\n\n";

    int tracks_match = match_tracks(tracks, "el", true);
    cout << "number of matching tracks: " << tracks_match << "\n\n";
    int artists_match = match_artists(tracks, "el", true);
    cout << "number of matching artists: " << artists_match << "\n\n";
    int cds_match = match_cds(tracks, "el", true);
    cout << "number of matching cds: " << cds_match << "\n\n";
    cout << "number of cds: " << number_of_cds(tracks);
