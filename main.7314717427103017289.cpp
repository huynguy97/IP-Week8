...
*/
    if ( length.minutes >= 0  && length.seconds >= 0 && length.seconds <= 59)
        ...
        cout << " Length does not conform to normal notation. " ;
    return out ;
}
...
*/
    int number = 0 ;
    for (int i = 0; i < Tsize(tracks) ; i++)
    {
        Track vtrack = tracks [i] ;
        if ( match (track, vtrack.title) )
        {
        number++ ;
        }
    }
...
*/
    vector<string> different_names ;
    for ( int i = 0 ; i < Tsize(tracks) ; i++ )
    {
        Track vtrack = tracks [i] ;
        different_names.push_back(vtrack.cd);
        int match = 0 ;
        for ( int j = 0 ; j < Ssize(different_names) ; j++ )
        {
            if ( vtrack.cd == different_names [j] )
                match++;
        }
        if ( match == 0 )
            different_names.push_back(vtrack.cd) ;
    }
    return Ssize(different_names) ;
}

void scan_database ( vector<Track>& tracks )
{//              
    assert (true);
/*                
                                                                                         
*/
    getline ( cin, string sub ) ;
}
