...
const int MAX_INFO_LINES = 100000;
...
*/
    char dummy; //                                
    in >> length.minutes >> dummy >> length.seconds;
    return in;
}
...
int match_artists(vector<Track>& tracks, string artist, bool display)
{
    ...
    //                                                                 
    int found_artists = 0;
    TrackDisplay artist_true = {true, false, false, false, false, false, false, false};
    for(int i = 0;i<tracks.size();i++)
    {
        if(match(tracks[i].artist, artist))
        {
            bool go_ahead = true;
            //                                                                                              
            for(int p = 0;p<i;p++)
            {
                if(tracks[i].artist == tracks[p].artist)
                {
                    go_ahead = false;
                    break;
                }
            ...
            if(go_ahead)
            {
                ...
                if(display)
                {
                    show_track(tracks[i], artist_true);
                }
            }
        }
    }
...
int match_cds(vector<Track>& tracks, string artist, bool display)
{
    ...
    //                                                                 
    int found_CD = 0;
    TrackDisplay CD_true = {true, true, true, false, false, false, false, false};
    for(int i = 0; i<tracks.size();i++)
    {
        if(match(tracks[i].artist, artist))
        {
...
bool read_file(ifstream& infile, vector<Track>& tracks, char filename[11], bool display)
{
    //               
    assert(true);
    //                
    ...
    if(infile)
    {
        while(infile)
        {
            infile.getline(trackarray, 1000, '\n');
            switch(i%8)
            {
                case 0: track.artist = trackarray;
                case 1: track.cd = trackarray;
                case 2: track.year = int(trackarray);
                case 3: track.track = int(trackarray);
                case 4: track.title = trackarray;
                case 5: track.tags = trackarray;
                case 6: track.time = {int(trackarray[0]), int(trackarray[2])};
                case 7: track.country = trackarray;
            }
            if(i%8 == 7)
            {
        ...
        infile.close();
        cout << "The amount of tracks is: " << i+1 << endl;
        return true;
    }
    return false;
}
...
*/
    ...
    Length l3 = {3,21};
    Length l4 = {3,14};
    cout << operator+(l1,l2) << endl;
    cout << operator+(l1,l3) << endl;
    cout << operator+(l1,l4) << endl;
    cout << operator+(l2,l3) << endl;
    cout << operator+(l2,l4) << endl;
    cout << operator+(l3,l4) << endl;

    ifstream infile;
    char filename[11] = "Tracks.txt";
    vector<Track> complete_tracks;
    if(read_file(infile, complete_tracks, filename, false))
    {
        cout << "The amount of different Cd's is: " << number_of_cds(complete_tracks) << endl;
    }
    else{
        cout << "Something went wrong while reading the file. Please try again later.\n";
    }
