...
*/
    string value;
    getline(in, value, ':');        //                                                                                                                  
    length.minutes = stoi(value);
    getline(in, value);
    length.seconds = stoi(value);
    return in;
}
...
{
//               
    assert(tracks.size() > 0 && track.length() > 0);
//                                                                                               
//                                                                                        
    int tracksfound = 0;
    TrackDisplay showing = {true, true, true, true, true, true, true, true};
...
{
//               
    assert(tracks.size() > 0 && artist.length() > 0);
//                                                                                                    
//                                                                                        
    int artistsfound = 0;
    string lastfoundartist;
    TrackDisplay showing = {true, false, false, false, false, false, false, false};

    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(artist, tracks[i].artist) && tracks[i].artist != lastfoundartist)
        {
            artistsfound++;
            lastfoundartist = tracks[i].artist;
            if (display)
                show_track(tracks[i], showing);
        }
    }
...
//                                                                                                                         
    int cdsfound = 0;
    string lastfoundcd;
    TrackDisplay showing = {true, true, true, false, false, false, false, false};

    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(artist, tracks[i].artist) && tracks[i].cd != lastfoundcd)
        {
            cdsfound++;
            lastfoundcd = tracks[i].cd;
            if (display)
                show_track(tracks[i], showing);
        }
    }
...
//                                                                           
    int cdsfound = 1;
    string lastfoundcd = tracks[0].cd;
    for (int i = 0; i < testDB.size(); i++)
    {
        if (tracks[i].cd != lastfoundcd)
        {
            cdsfound++;
            lastfoundcd = tracks[i].cd;
        }
...
*/
    string year, tracknr, endtest;

    getline(in, endtest);
    if (endtest == "")
        return in;

    track.artist = endtest;
    getline(in, track.cd);
    getline(in, year);
    getline(in, tracknr);
...
*/
    cout << "Tracks imported: " << read_tracklist() << endl;
	return 0;
}
