...
*/
    ...
    td.showTrack = true;
    td.showYear = true;
    if(td.showAlbum && td.showArtist && td.showCountry && td.showLength && td.showTags && td.showTitle && td.showTrack && td.showYear){
        cout << track.artist << " "
             << track.cd << " "
             << track.year << " "
             << track.track << " "
...
int matchTracks (vector<Track>& tracks, string track, bool display){
    //             
    assert(true);
/*                
