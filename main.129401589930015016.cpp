...
*/
    Length new_length;
    new_length.minutes = (a.seconds + b.seconds) / 60 + a.minutes + b.minutes;
    new_length.seconds = (a.seconds + b.seconds) % 60;
    return new_length;
}
...
*/
    int match_counter = 0;
    TrackDisplay lt = {display, display, display, display, display, display, display, display};
...
*/
    string new_artist = "";
    int match_counter = 0;
    TrackDisplay lt = {display,false,false,false,false,false,false,false};

    for (int i = 0; i < tracks.size(); i++)
        if (match(artist,tracks[i].artist) && tracks[i].artist != new_artist)
        {
            new_artist = tracks[i].artist;
            show_track(tracks[i],lt);
            match_counter++;
        }
...
*/
    string new_cd = "";
    int match_counter = 0;
    TrackDisplay lt = {display, display, display, false, false, false, false, false};

    for (int i = 0; i < tracks.size(); i++)
        if ( match(artist,tracks[i].artist) && tracks[i].cd != new_cd )
        {
            new_cd = tracks[i].cd;
            show_track(tracks[i],lt);
            match_counter++;
...
void read_file (ifstream& infile, vector<Track>& DataBase)
{//              
...
*/
    ...
    read_file(infile,DataBase);
    cout << "The number of tracks stored in the database is: " << DataBase.size() << endl;
	return 0;
}
