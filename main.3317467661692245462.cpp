...
*/
    ...
    for(int i=0;i<tracks.size();i++){
        if(match(artist,tracks[i].artist)&&tracks[i].artist!=tracks[i-1].artist){
            a++;
            string found[a]=tracks[i].artist;
            if(display){
...
void read_file(ifstream& infile,vector<Track>& full_list)
{
    ...
    while(infile){
        Track a;
        infile>>a;
        full_list.push_back(a);
        TrackDisplay lt = {true,true,true,true,true,true,true,true};
        show_track(full_list[i],lt);
        cout<<endl;
        i++;}
}
...
*/
    string filename;
    ifstream infile;
    vector<Track> full_list;
    if(open_file(infile,filename)){
        cout<<"File loaded";
        read_file(infile,full_list);}
return 0;
}
