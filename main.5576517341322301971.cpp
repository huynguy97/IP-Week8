...
{//              
    ...
    sum.seconds = a.seconds + b.seconds - ((a.seconds+b.seconds)/60)*60;
    return sum;
/*                
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    assert(true);
    int vectorlength = tracks.size();
    vector<string> repitition;
    TrackDisplay trackdisplay = {true, false, false, false, false, false, false, false};
    int i = 0;
    int counter = 0;
    while (i < (vectorlength))
    {
        int j = 0;
        int repcounter = 0;
        while(j < repitition.size()) //                                            
        {
            if (match(tracks[i].artist, repitition[j]))
            {
                repcounter++;
            }
            j++;
        }
        if (match(artist, tracks[i].artist) && repcounter == 0)
        {
            if (display)
            {
                show_track(tracks[i], trackdisplay);
            }
            repitition.push_back(tracks[i].artist); //                                        
            counter++;
        }
        i++;
    }
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    int vectorlength = tracks.size();
    vector<string> repitition;
    TrackDisplay trackdisplay = {true, true, true, false, false, false, false, false};
    int i = 0;
    int counter = 0;
    while (i < (vectorlength))
    {
        ...
        while(j < repitition.size())
        {
            if (match(tracks[i].cd, repitition[j]))
            {
                repcounter++;
            }
            j++;
        ...
        if (match(artist, tracks[i].artist) && repcounter == 0)
        {
            ...
            repitition.push_back(tracks[i].cd);
            counter++;
        }
        i++;
    }
...
int readtracks (ifstream& inputfile, vector<Track>& trackinput)
{
    assert(inputfile);
    ...
    while(inputfile)
    {
        inputfile >> trackinput[counter];
        counter++;
    }
    int i = 0;
    while(i<counter-1)
    {
        show_track(trackinput[i], trackdisplay);
        i++;
    }
    return counter-1;
/*                
...
{//              
    assert (true) ;
    ifstream inputfile;
    vector<Track> trackinput(9999);
    int numbersread;
    numbersread = readtracks(inputfile, trackinput);
    cout << "A total of " << numbersread << " tracks have been read.";
/*                
