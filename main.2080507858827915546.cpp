...
Length operator+ (const Length& a, const Length& b)
{
    ...
    */
    ...
    if(output.seconds >= 60)
    {
        output.minutes = 1;
        output.seconds = output.seconds - 60;
    }
    output.minutes = output.minutes + a.minutes + b.minutes;
    return output;
}
...
istream& operator>> (istream& in, Track& tracky)
{
    //              
    assert (true) ;
    /*                
    ...
    */
    getline(in,tracky.artist);
    getline(in,tracky.cd);
    in >> tracky.year;
    in >> tracky.track;
    in.ignore(1, '\0');
    getline(in,tracky.title);
    getline(in,tracky.tags);
    in >> tracky.time;
    in.ignore(1, '\0');
    getline(in,tracky.country);
    in.ignore(1, '\n');
    return in;
}
...
*/
    ...
    for(int i = 0; i < tracks.size(); i++)
    {
        if(match(artist,tracks[i].artist) && BlackList.end() == find(BlackList.begin(),BlackList.end(),tracks[i].artist))
        {
            counter++;
            if(display)
            {
                show_track(tracks[i], {true});
                BlackList.push_back(tracks[i].artist);
            }
        }
    }
    return counter;
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    */
    ...
    for(int i = 0; i < tracks.size(); i++)
    {
        if(match(artist,tracks[i].artist) && BlackList.end() == find(BlackList.begin(),BlackList.end(),tracks[i].cd))
        {
                BlackList.push_back(tracks[i].cd);
                counter++;
            if(display)
            {
                show_track(tracks[i], {true, true, true});
            }
        }
    }
    return counter;
...
int number_of_cds (vector<Track>& tracks)
{
    ...
    for(int i = 0; i < tracks.size(); i++)
    {
        if(BlackList.end() == find(BlackList.begin(),BlackList.end(),tracks[i].cd))
        {
            counter++;
            BlackList.push_back(tracks[i].cd);
        }
...
int main()
{
    ...
    */
    ...
    const int TOTAL_TRACKS = 5891;
    vector<Track> TrackDB(TOTAL_TRACKS);
    ifstream inputfile;
    ...
    inputfile.close();
    cout << "Total tracks read: " << TrackDB.size() << endl;
