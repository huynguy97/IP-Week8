...
{//              
	...
	*/
	Length newlength;
	newlength.minutes = a.minutes + b.minutes;
	newlength.seconds = a.seconds + b.seconds;
	if (newlength.seconds > 59) {
...
int match_tracks(vector<Track>& tracks, string track, bool display)
{
	...
	int occurences = 0;
	TrackDisplay takeitall = { true,true,true,true,true,true,true,true };
	for (int i = 0; i < tracks.size(); i++) {
		if (match(track, tracks[i].title)) {
			occurences++;
			if (display) {
				show_track(tracks[i], takeitall);
				cout << '\n';
			}
		}
	}
...
int match_artists(vector<Track>& tracks, string artist, bool display) {
	...
	//                
	//                                                         
	vector<Track> foundartists;
	 

	int occurences = 0;
	TrackDisplay showartsist = { true };
	for (int i = 0; i < tracks.size(); i++) {
		if (match(artist, tracks[i].artist)) {

			bool foundalready = false;
			for (int j = 0; j < foundartists.size(); j++) {
				if (match(artist, foundartists[j].artist)) foundalready = true;
			}

			if (!foundalready) {
				occurences++;
				if (display) {
					show_track(tracks[i], showartsist);
					foundartists.push_back(tracks[i]);
					cout << '\n';
				}
			}
		}
...
int match_cds(vector<Track>& tracks, string artist, bool display) {
	...
	int occurences = 0;
	TrackDisplay showartsist = { true,true,true };
	for (int i = 0; i < tracks.size(); i++) {
		if (match(artist, tracks[i].artist)) {

			bool foundalready = false;
			for (int j = 0; j < foundartists.size(); j++) {
				if (foundartists[j].cd == tracks[i].cd) foundalready = true;
			}
...
int number_of_cds(vector<Track>& tracks) {
	...
	vector<Track> foundtracks;
	int occurances = 0;
	for (int i = 0; i < tracks.size(); i++) {
		bool foundalready = false;
		for (int j = 0; j < foundtracks.size(); j++) {
			if (tracks[i].cd == foundtracks[j].cd) foundalready = true;
		}
		if (!foundalready) {
			occurances++;
			foundtracks.push_back(tracks[i]);
		}
	}
	return occurances;
}
...
{//              
	...
	*/
	...
	ifstream testfile;
	testfile.open("Tracks.txt");
	vector<Track> tracks;

	while (testfile && !testfile.eof()) {
		Track ttrack;
		testfile >> ttrack;
	...
	TrackDisplay tdsp = { true,true,true,true,true,true,true,true };
	for(Track t : tracks)
	{
		show_track(t, tdsp);
		cout << "\n" << endl;
