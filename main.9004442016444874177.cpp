...
*/
    ...
    int a_total_seconds = 0;
    int b_total_seconds = 0;
    int new_total_seconds = 0;

    a_total_seconds = a.minutes * 60 + a.seconds;
    b_total_seconds = b.minutes * 60 + b.seconds;

    new_total_seconds = a_total_seconds + b_total_seconds;

    sum.minutes = new_total_seconds / 60;
    sum.seconds = new_total_seconds % 60;
...
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    cout << "The number of matching tracks is " << no_of_tracks << "." << endl;
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        if (i == 0 && match(artist, tracks[i].artist))
        {
            no_of_artists++;
        ...
        if (i >= 1 && match(artist, tracks[i].artist) && !match(artist, tracks[i-1].artist))
        {
            no_of_artists++;
    ...
    cout << "The number of matching artists is " << no_of_artists << "." << endl;

    return no_of_artists;
}
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        if (i >= 1 && match(artist, tracks[i].artist) && !match(tracks[i].cd, tracks[i-1].cd))
        {
            no_of_cds++;
    ...
    cout << "The number of matching cds is " << no_of_cds << "." << endl;
...
int number_of_cds (vector<Track>& tracks)
{
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        if (i >= 1 && !match(tracks[i].cd, tracks[i-1].cd))
        {
            no_of_cds++;
        }
    }

    cout << "The number of cds is " << no_of_cds << "." << endl;
...
*/
    ...
    vector<Track> shrek(20000); //                                              

    int i = 0;
    Track track;
    TrackDisplay td = {true, true, true, true, true, true, true, true};

    while (infile >> shrek[i])
    {
        show_track(shrek[i], td); //                                                      
        cout << endl;

        i++;
    }

    cout << "\n\nNumber of tracks implemented: " << i << endl;
