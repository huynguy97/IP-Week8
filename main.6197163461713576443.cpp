...
int match_tracks(vector<Track>& tracks, string track, bool display)
{
    ...
    for(int i =  0; i < tracks.size(); i++ ){
        if (match(track, tracks[i].title)){ //                                                                  
            ...
            if(display)
                {
                cout <<  show_track(tracks[i], lt) << endl;
                }
    if(!display)
    {
        cout << "The number of tracks that contain the string" <<
        track << "is" << coun_t;
    }
    return coun_t;
...
int match_artists(vector<Track>& tracks, string artist, bool display)
{
    ...
    for(int i = 0; i < tracks.size(); i++)
    {
        ...
        {
            check.push_back(artist)
            for (int j = 0; j < check.size(); j++) //                                            
            {
                if (check[j] != tracks[i].artist)
                {
                    coun_t++; //                               
                    if(display)
                        cout << show_track(tracks[i], lt) << endl;
...
int match_cds(vector<Track>& tracks, string artist, bool display)
{
    ...
    for(int i =  0; i < tracks.size(); i++ )
    {
        if(match(artist, tracks[i].artist)) //                                     
        {
            check.push_back(tracks[i].cd);
            for (int j = 0;  j< check.size(); j++)
            {
                if (check[j] != tracks[i].cd) //                                        
                {
                    coun_t++;
                    if(display)
                        cout << show_track(tracks[i], lt) << endl;
...
int number_cds(vector<Track>& tracks)
{
    int coun_t = 0;
    vector<char> check;
    for(int i =  1; i < tracks.size(); i++) //                                          
        if(tracks[count_t].cd != tracks[i].cd)
            check.push_back(tracks[i].cd); //                                      
            for (int j = 0; j < check.size(); j++)
            {
                if (check[j] != tracks[i].cd)
    ...
    cout << "There are" << coun_t << "different CD's"
    return coun_t;
}
...
*/
istream& operator>> (istream& in, Track& tr)
{
    int count_entries = 0;
    while (in) //                                                      
    {
        getline(in, tr.artist);
        in.ignore();
        getline(in, tr.cd);
        in.ignore();
        in >> tr.year;
