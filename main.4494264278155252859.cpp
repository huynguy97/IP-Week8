...
*/
    char t;
    in >> length.minutes >> t >>length.seconds;
    return in;
}
...
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    {
        if (match(track, tracks[i].title) && display)
        {
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(artist, tracks[i].artist) && tracks[i].artist != tracks[i-1].artist && display)
        {
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(artist, tracks[i].artist) && tracks[i].cd != tracks[i-1].cd && display)
        {
...
*/
    ...
    ifstream inputfile;
    open_file(inputfile, "Tracks.txt");
    Track tracks;
    int n = 0;
    while (inputfile)
    {
