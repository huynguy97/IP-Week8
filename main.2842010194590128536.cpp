...
{
    //              
    assert (length.minutes >= 0 && length.seconds <= 59 && length.seconds >= 0);
    /*                
                                                                                              
    */
    string optionalZeroMinutes = "";
    string optionalZeroSeconds = "";
    if (length.minutes < 10)
    {
        optionalZeroMinutes = "0";
    }
    if (length.seconds < 10 )
    {
        optionalZeroSeconds = "0";
    }

    out << optionalZeroMinutes << length.minutes << ':'
        << optionalZeroSeconds << length.seconds ;
    return out ;
...
int match_artists (vector <Track>& tracks, string artist, bool display)
{
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        {
            bool isInArtists = false;
            for (int j = 0; j < artists.size() && !isInArtists; j++)
            {
    ...
    cout << "The amount of artists found is: " << artists.size() << endl;
    return artists.size();
}
...
int match_tracks (vector <Track>& tracks, string track, bool display)
{
    ...
    cout << "The amount of tracks found is: " << amountOfTracks << endl;
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    for ( int j = 0; j < artists.size(); j ++ )
    {
        for ( int i = 0; i < tracks.size(); i++)
        {
            ...
            {
                bool isInCds = false;
                for (int k = 0; k < cds.size() && !isInCds; k++)
                {
                    if (tracks[i].cd == cds[k])
                    {
                        isInCds = true;
                    }
                }
    ...
    cout << "The amount of cds found is: " <<cds.size() << endl;
    return cds.size();
...
void readFile(vector<Track>& tracks) {
    //             
    assert(true);
    //              
    //                                                               
