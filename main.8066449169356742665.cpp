...
*/
    ...
    getline(in,whiteline);
    return in;
}
bool check_before_title(int from, vector<Track>& tracks)
{
    assert(from >= 0);
    //               
    //                                                              
    ...
    {
        if(match(tracks[from].title,tracks[i].title))
            return true;
    }
    return false;
}
bool check_before_artist(int from, vector<Track>& tracks)
{
    assert(from >= 0);
    //               
    //                                                                
    for(int i = from -1; i >= 0; i--)
    {
        if(match(tracks[from].artist,tracks[i].artist))
            return true;
    }
    return false;
}
bool check_before_cds(int from, vector<Track>& tracks)
{
    assert(from >= 0);
    //               
    //                                                              
    for(int i = from -1; i >= 0; i--)
    {
        if(match(tracks[from].cd,tracks[i].cd))
            return true;
    }
    return false;
}
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    {
        if(match(track,tracks[i].title) && !check_before_title(i,tracks))
        {
            no_of_matches++;
            if(display)
            {
                TrackDisplay showall = {true,true,true,true,true,true,true,true};
                show_track(tracks[i],showall);
            }
        }
    }
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    for(int i= 0 ; i < tracks.size();i++)
    {
        if(match(artist,tracks[i].artist) && !check_before_artist(i,tracks))
        {
            no_of_matches++;
            if(display)
            {
                TrackDisplay showart = {true,false,false,false,false,false,false,false};
                show_track(tracks[i],showart);
            }
        }
    }
    return no_of_matches;
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    for(int i= 0 ; i < tracks.size();i++)
    {
        if(match(artist,tracks[i].artist) && !check_before_cds(i,tracks))
        {
            no_of_matches++;
            if(display)
            {
                TrackDisplay showcd = {true,true,true,false,false,false,false,false};
                show_track(tracks[i],showcd);
            }
        }
    }
    return no_of_matches;
...
int number_of_cds(vector<Track>& tracks)
{
    ...
    for(int i = 0; i < tracks.size();i++)
    {
        if(!check_before_cds(i,tracks))
            no_of_cds++;
    }
    return no_of_cds;
}
bool read_in(int& no, ifstream& infile, vector<Track>& tracks, char filename [11],bool display)
{
    assert(no == 0);
    //                
    //                                           
...
*/
    cout << "For the first part of the assignment" << endl;
    ...
    cout << "The following is to test the required implementations: " << endl;
    cout << "The amount of found tracks with 'el' in the title is : " << match_tracks(testDB,"el",false) << endl;
    cout << "The amount of found artist with 'el' in their name is: " << match_artists(testDB,"el",false) << endl;
    cout << "The amount of Cd's from artist with 'el' in their name is: " << match_cds(testDB,"el", false) << endl;
    cout << "The total amount of Cd's in the testdatabase is: " << number_of_cds(testDB) << endl;
    cout << endl;

    ifstream infile;
    char filename [11] = "Tracks.txt";
    vector<Track> filetracks;
    int nom = 0;
    if(!read_in(nom,infile,filetracks,filename,false))
        {
            cout << "Something went wrong opening the file." << endl;
            return 0;
        }
    cout << "The amount of tracks is: " << nom + 1 << endl;
    cout << "The amount of different Cd's is: " << number_of_cds(filetracks) << endl;
    return 0;
}
