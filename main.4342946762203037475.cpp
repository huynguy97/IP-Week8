...
{//              
    assert (0 <= length.minutes && length.seconds < 100 && 0 <= length.seconds && length.seconds < 60) ;
/*                
                                                                                          
*/
    out<<(length.minutes<10?"0":"")<<length.minutes<<':'<<(length.seconds<10?"0":"")<<length.seconds;
    return out;
}
...
*/
    Length ret={a.minutes+b.minutes+(a.seconds+b.seconds)/60,(a.seconds+b.seconds)%60};
    return ret;
}
...
bool contains(vector<string> arr, string val){
    //              
    assert (true) ;
    /*                                                                        */
    for(string el:arr){
...
int match_artists (vector<Track>& tracks, string artist, bool display){
    ...
    /*                                                                                                     
                                                                                   */
    const TrackDisplay td={true,false,false,false,false,false,false,false};
    vector<string> matches;
    for(Track t:tracks){
        if(match(artist,t.artist)){
            if(!contains(matches,t.artist)){
                matches.push_back(t.artist);
                if(display){
                    show_track(t,td);
                }
            }
...
int match_cds (vector<Track>& tracks, string artist, bool display){
    ...
    /*                                                                                                
                                                                                                                   */
    const TrackDisplay td={true,true,true,false,false,false,false,false};
    vector<string> matches;
    for(Track t:tracks){
        if(match(artist,t.artist)){
            if(!contains(matches,t.cd)){
                matches.push_back(t.cd);
                if(display){
                    show_track(t,td);
                }
            }
