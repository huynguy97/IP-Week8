...
*/
    ...
    int new_seconds;
    Length new_length;
        new_minutes += (a.minutes + b.minutes);
        new_seconds += (a.seconds + b.seconds);
    if(new_seconds > 59){
        new_minutes ++;
        new_seconds -= 60;
    }
    new_length.minutes = new_minutes;
    new_length.seconds = new_seconds;
    return  new_length;
}
...
int match_artists (vector<Track>& tracks, string artist, bool display){
   ...
   */
    ...
    string found_artist = "         ";
    for(int i = 0; i < tracks.size(); i++){
        if(match(artist, tracks[i].artist) && !match(found_artist, tracks[i].artist)){
            if(display){
                lt.showArtist = true;
                lt.showAlbum = false;
...
int match_cds (vector<Track>& tracks, string artist, bool display){
    ...
    */
    ...
    string found_cd = "                               ";
    for(int i = 0; i < tracks.size(); i++){
        if(match(artist, tracks[i].artist) && !match(found_cd, tracks[i].cd)){
            if(display){
                lt.showArtist = true;
                lt.showAlbum = true;
                lt.showCountry = false;
...
//     
    for(int i = 0; i < tracks.size(); i++){
        for(int j = 0; j < cds.size(); j++){
            if(!found_bool){
                found_bool = match(found_cd, tracks[i].cd);
            }
        }
        if(!found_bool){
            found ++;
...
*/
    ...
    while(infile){
        ...
        show_track(tracks[number], lt);
        number ++;
    }
    cout << "Number :" << number << endl;
    cout << endl << "The queries of p3:"<< endl;
    cout << "Match_tracks:" << endl;
    match_tracks(testDB, "el", true);
    cout << "Match_artists:" << endl;
    match_artists(testDB, "el", true);
    cout << "Match_cds:" << endl;
    match_cds(testDB, "el", true);
    cout << "Number_of_cds:" << endl;
    cout << "Nr: " << number_of_cds(testDB);
	return 0;
}
