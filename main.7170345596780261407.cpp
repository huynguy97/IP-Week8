...
*/  
    int amin = a.minutes;
    int bmin = b.minutes;
    int minsum = amin + bmin;
    int sumsec = a.seconds + b.seconds;
    if(sumsec > 60){minsum += 1;} //                                                        
    int asec = sumsec % 60; //                            
    Length retLength;
    retLength.minutes = minsum;
    retLength.seconds = asec;
    return retLength;
}
...
int match_tracks(vector<Track>& tracks, string track, bool display){
    int matches = 0;
    TrackDisplay tr1 = {true, true, true, true, true, true, true, true};
    if(display){
        for(int i = 0; i< tracks.size(); i++){
            if(match(track, tracks.at(i).title)){
                matches++;
                show_track(tracks.at(i), tr1);
            }
        }
    }
    return matches;
...
int match_artists(vector<Track>& tracks, string artist, bool display){
    int matches = 0;
    TrackDisplay tr1 = {true, false, false, false, false, false, false, false};
    vector<string> vec;
    if(display){
        for(int i = 0; i< tracks.size(); i++){
...
bool readDB(vector <Track>& MusDB){
    ifstream infile;
    ...
    while(infile){
        ...
        MusDB.push_back(elem);
    }
    return true;
}

void showDB(vector <Track> db){
    for(int i = 0; i < db.size(); i++){
        TrackDisplay tr1 = {true, true, true, true, true, true, true, true};
        show_track(db.at(i), tr1);
    }
}
int main()
{//              
