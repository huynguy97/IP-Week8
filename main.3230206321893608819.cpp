...
*/
    ...
    for(int i = 0; i < tracks.size(); i++){
        if(match(artist, tracks[i].artist)){
            ...
            if(valid) {
                counter++;
                check.push_back(tracks[i].artist);
                if (display) {
...
int readfile(ifstream& infile, string filein, vector<Track>& tracks)
{//              
...
*/
    ...
    vector<Track> tracks;
    cout << readfile(input_file, filename, tracks);
