    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        if (match(artist, tracks[i].artist))
        {
            if (tracks[i+1].cd != tracks[i].cd)
                occurance++;
            if (tracks[i+1].cd != tracks[i].cd && display){
...
int main()
{
    ...
    */
    ...
    cout << tracks.size() << endl << endl;

    cout << match_tracks (tracks, "hoi", true);
    cout << match_artists(tracks, "hoi", true);
    cout << match_cds(tracks, "hoi", true);
