...
int MatchTracks (vector<Track>& archive, string track, bool display)
{
//               
    assert (true);
/*                
...
bool CheckRecurrence(vector<string>& container, string match)
{
//               
    assert (true);
/*                
...
*/
    for (const string n : container)
        if (n == match)
            return true;
    return false;
}

int MatchArtists (vector<Track>& archive, string artist, bool display)
{
//               
    assert (true);
/*                
...
*/
    ...
    for (const Track t : archive)
    {
        if (match(artist, t.artist) && t.artist != lastArtist && !CheckRecurrence(artists, t.artist)) //                                    
                                                                                                      //                                         
                                                                                                      //                        
        {
            lastArtist = t.artist;
            artists.push_back(t.artist); //                                                  
            if (display)
            {
                show_track(t, lt);
                cout << endl;
...
int MatchCDs (vector<Track>& archive, string artist, bool display)
{
//               
    assert (true);
/*                
...
*/
    ...
    for (const Track t : archive)
    {
        if (match(artist, t.artist) && t.cd != lastCD && !CheckRecurrence(CDs, t.cd))
        {
            lastCD = t.cd;
            CDs.push_back(t.cd);
            if (display)
...
int NumberOfCDs (vector<Track>& archive)
{
//               
    assert (true);
/*                
...
*/
    ...
    cout << ReadFile(allTracks, inputfile) << endl;
    cout << MatchCDs(allTracks, "el", true);
    inputfile.close();
	return 0;
}
