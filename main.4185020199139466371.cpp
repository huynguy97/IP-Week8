...
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    */
    ...
    int vsize = size(tracks) ;
    int count = 0;

    TrackDisplay Display = { true, true, true, true, true, false, true, true};

    for (int i=0; i<vsize; i++)
        {
            display = match(track,tracks[i].title);

            if(display)
            {
                count++;
                show_track(tracks[i], Display);
            }
        }
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    */
    ...
    //                                                                                                 
    vector<Track> Storing ;

    TrackDisplay Display = { true, false, false, false, false, false, false, false};

    for (int i=0; i<vsize; i++)
        {
            display = match(artist,tracks[i].artist);

            if(display)
            {
                int ok=1;

                for(int j=0; j<size(Storing)&&ok; j++)
                    if(Storing[j].artist == tracks[i].artist)
                        ok = 0;

                if(ok == 1)
                    {
                        Storing.push_back(tracks[i]);
                        count++;
                    }
            }
        }

    for(int j=0; j<size(Storing); j++)
        show_track(Storing[j], Display);
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    */
    ...
    TrackDisplay Display = { true, true, true, false, false, false, false, false};
...
*/
    ...
    match_tracks(testDB, "el", false);
    match_artists(testDB, "el", false);
    match_cds(testDB, "el", false);
