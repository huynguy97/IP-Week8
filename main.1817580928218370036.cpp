...
*/
    out << length.minutes << ':' << length.seconds / 10 << length.seconds % 10 ;
    return out;
}
...
*/
    ...
    result.minutes = a.minutes + b.minutes + (seconds / 60);
...
*/
    ...
    for(int i = 0; i < vector_size; i++)
    {
        if(match(artist, tracks[i].artist) && !contains(artists, tracks[i].artist))
        {
            artists.push_back(tracks[i].artist);
            if(display)
            {
                show_track(tracks[i], {1,0,0,0,0,0,0,0});
                cout << endl;
            }
        }
    }
...
*/
    ...
    for(int i = 0; i < vector_size; i++)
    {
        if(match(artist, tracks[i].artist) && !contains(cds, tracks[i].cd))
        {
            cds.push_back(tracks[i].cd);
            if(display)
            {
                show_track(tracks[i], {1,1,1,0,0,0,0,0});
                cout << endl;
            }
        }
    }
...
void test_match_tracks()
{
    int found = match_tracks(testDB, "el", true);
    cout << "expected 4 tracks" << endl;
    cout << "found " << found << " tracks" << endl;
}

void test_match_artists()
{
    int found = match_artists(testDB, "el", true);
    cout << "expected 2 artists" << endl;
    cout << "found " << found << " artists" << endl;
}

void test_match_cds()
{
    int found = match_cds(testDB, "el", true);
    cout << "expected 3 albums" << endl;
    cout << "found " << found << " albums" << endl;
}
...
*/
    ...
    while(input_file)
    {
        ...
        if(input_file)
        {
            ...
            show_track(track, {1,1,1,1,1,1,1,1});
            cout << endl;
        }
    }

    cout << "============================================"<< endl;;
    cout << "Expected tracks to read: 5891." << endl;
    cout << "Number of tracks read: " << tracks.size() << "." << endl;
