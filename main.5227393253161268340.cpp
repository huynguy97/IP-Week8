...
void open_and_read (ifstream& file, Track database,vector<Track>& database_main)
{
    assert(true);
//                                                               
    while(file)
    {
        file >> database;
        database_main.push_back(database);
    }
}
...
*/
    cout << length.minutes << ":" << length.seconds << endl;
}
...
*/
    ...
    if(lt.showLength)
    {
        cout << track.time.minutes << ":" << track.time.seconds <<endl;
    }
    if(lt.showCountry)
    {
...
//                                              
    ...
    dis.showAlbum = false;
    dis.showArtist = true;
    dis.showCountry = false;
    dis.showLength = false;
    dis.showTags = false;
    dis.showTitle = false;
    dis.showTrack = false;
    dis.showYear= false;
    for (int i = 0; i < testDB.size(); i++)
    {
        ...
        if (match(artist,tracks[i].artist)&&current_artist!=tracks[i].artist)
        {
            current_artist=tracks[i].artist;
            counter++;
            if (display)
            {
                show_track (testDB[i],dis);
                cout <<endl;
            }
        }
    }
...
//                                                              
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        if (match(artist,tracks[i].artist)&&current_cd!=tracks[i].cd)
        {
            current_cd=tracks[i].cd;
            counter++;
            if (display)
            {
                show_track (tracks[i],dis);
...
int number_of_cds (vector<Track>& tracks)
{
    assert(true);
//                               
    string current_cd=tracks[0].cd;
    int counter=1;
    for (int i = 0; i < tracks.size(); i++)
    {
        if (current_cd != tracks[i].cd)
        {
            current_cd=tracks[i].cd;
            counter++;
        }
...
*/
    ...
    Track database;
    vector<Track> database_main;
    open_and_read (file,database,database_main);
    match_tracks (database_main, part_of, display_value);
    cout <<endl;
    match_artists (database_main, part_of, display_value);
    cout << endl;
    match_cds(database_main, part_of, display_value);
    cout << endl;
    cout << number_of_cds (database_main);
	return 0;
}
