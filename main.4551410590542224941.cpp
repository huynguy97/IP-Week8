...
*/
    Length len = {a.minutes + b.minutes, a.seconds + a.minutes};
    if (len.seconds > 59)
    {
        len.seconds - 60;
        len.minutes++;
...
*/
    ...
    getline(in, track.cd);
    getline(in, year_string);
    track.year = atoi(year_string.c_str());
    getline(in, track_string);
    track.track = atoi(track_string.c_str());
    getline(in, track.title);
    getline(in, track.tags);
    in >> track.time;
    //
    getline(in, empty_string);
...
*/
    string lastCountedArtist = ";{"; //                                  
    int no_of_matches = 0;
    for (Track track : tracks) {
        if (!match(lastCountedArtist, track.artist) && match(artist, track.artist)) {
            lastCountedArtist = track.artist;
            no_of_matches++;
            if (display) {
...
*/
    ...
    string lastCountedCDtitle = ";{"; //                                  
    int no_of_matches = 0;
    for (Track track : tracks) {
        if (match(artist, track.artist) && !match(lastCountedCDtitle, track.cd)) {
            lastCountedCDtitle = track.cd;
            no_of_matches++;
            if (display) {
...
*/
    ...
    int no_of_matches = 0;
    for (Track track : tracks) {
        if (!match(lastCountedCDtitle, track.cd)) {
            lastCountedCDtitle = track.cd;
            no_of_matches++;
        }
    }
