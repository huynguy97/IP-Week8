...
*/
    if (source.find(sub) != string::npos) {
        return true;
    } else {
        return false;
    }
...
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    //                                                                                                                                                  
    int number_found = 0;
    if (match(track, vector<Track>& tracks))
    {
        number_found++;
        if(display) {
        show_track(track, TrackDisplay);
        }
    }
    return number_found;
}
...
int number_of_cds (vector<Track>& tracks)
{
    ...
    string sub = "";
    while (searching) {
        if(tracks.find(sub) != string::npos) {
            cd_number++;
            sub = tracks.substr(tracks.find(sub), string::npos);
        } else {
            searching = false;
        }
    }
