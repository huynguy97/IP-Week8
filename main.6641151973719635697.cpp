...
*/
    ...
    sum.minutes = a.minutes + b.minutes + ((a.seconds + b.seconds - sum.seconds)/60);
    return sum;
...
*/
    ...
    if (lt.showYear)
    {
        cout << "Year:          " << track.year << endl << endl;
    }
...
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    for ( int i = 0; i < size(tracks); i++ )
    {
        ...
        if (match(sub,source))
        {
            ...
            if (display)
            {
               cout << "Information about the found track" << endl << endl;
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    for ( int i = 0; i < size(tracks); i++ )
    {
        ...
        if (match(sub,source))
        {
            ...
            if (display && tracks[i].artist != tracks[i+1].artist)
            {
...
int match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    for ( int i = 0; i < size(tracks); i++ )
    {
        ...
        if (match(sub,source))
        {
            ...
            if (display && tracks[i].cd != tracks[i+1].cd)
            {
...
int number_of_cds (vector<Track>& tracks)
{
    ...
    cout << "The total number of albums is: " << total_cd << endl;
}
...
int read_database (ifstream& input_file)
{
    //             
    assert(!input_file.is_open());
    //             
    //                                                                                                        
    vector<Track> file_storage;
    Track tmp;
    int counter = 0;
    ...
    {
        show_track(tmp, lt);
        file_storage.push_back(tmp);
        input_file >> tmp;
        counter ++;

    }
    cout << "The number of entries read:" << counter << endl;
    return counter;
}
int main()
{//              
