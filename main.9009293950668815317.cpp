...
TrackDisplay printAll = {true, true, true, true, true, true, true, true};
TrackDisplay printArtists = {true, false, false, false, false, false, false, false};
TrackDisplay printCds = {false, true, false, false, false, false, false, false};
...
*/
    if (0 <= length.minutes && 0 <= length.seconds && length.seconds <= 59){
        out << length.minutes <<':'
            << length.seconds ;
        return out ;
    }
...
*/
    int number_of_cor_tracks = 0;
    tracks.pop_back();
    for (int no = 0 ; no < tracks.size() ; no++)
    {
        if (match(tracks[no].title, track)){
            number_of_cor_tracks++;
            if (display) {
                show_track(tracks[no], printAll);
            }
        }
    }

    return number_of_cor_tracks ;
}
...
*/
    int number_of_artists = 0;
    tracks.pop_back();
    string last_artist;
    for (int no = 0 ; no < tracks.size() ; no++)
    {
        if (match(tracks[no].artist, artist) && tracks[no].artist != last_artist){
            number_of_artists++;
            last_artist = tracks[no].artist;
            if (display){
                show_track(tracks[no], printArtists);
            }
        }
    }
...
*/
    ...
    for (int no = 0 ; no < tracks.size() ; no++)
    {
        if (match(tracks[no].artist, artist) && tracks[no].artist != last_artist){
            last_artist = tracks[no].artist;
            number_of_matching_cds++;
            if (display){
                show_track(tracks[no], printCds);
            }
        }
    }

    return number_of_matching_cds;
}
...
*/
    int total_number_of_cds = 0;
    string cd_title;
    tracks.pop_back();
    for (int no = 0 ; no < tracks.size() ; no++)
    {
        if (tracks[no].cd == cd_title){
            total_number_of_cds++;
            cd_title = tracks[no].cd;
        }
    }
    return total_number_of_cds;
}
...
*/
    string name;
    cout << "Please enter the name of a music database: ";
    cin >> name ;
    int tracks = 0;
