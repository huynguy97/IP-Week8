...
*/
    ...
    sum.minutes = sum.minutes + ((a.seconds + b.seconds)/60); //                                                
    return sum;
}
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    for (int i = 0;i<size(tracks);i++)
    {
        ...
        {
            counter ++;
            if (i>=1&&tracks[i].artist == tracks[i-1].artist) //                                   
            {
                counter--; //                                                  
            }
            else
...
int  match_cds (vector<Track>& tracks, string artist, bool display)
{
    ...
    for(int i = 0; i<size(tracks);i++)
    {
        ...
        {
            counter++;
            if(i>=1&&tracks[i].cd==tracks[i-1].cd)//                               
            {
                counter--;//                                                                    
            }
            else
...
*/
    ...
    getline(in, track.cd);
    in >> track.year; getline(in,x);
    in >> track.track; getline(in, x);
    getline(in, track.title);
    getline(in, track.tags);
    in >> track.time; getline(in, x);
...
*/
    ...
    while(in)
    {
        ...
        if(track.artist!="") //                                                  
        {
            ...
            show_track(tracklist[i], td);
            cout << "\n";
            i++;
        }

    }
    cout << "The programme saved "<<size(tracklist)<<" songs."<< endl;
    in.close();
    Length l1 = {42, 42};
    Length l2 = {0,0};
    Length l3 = {3, 21};
    ...
    sum = l1 + l1;
    cout <<endl<< sum;
    cout << "Example tests for the functions:"<<endl;
    cout << endl << match_tracks(testDB, "el", true);
    cout << endl << match_artists(testDB, "el", true);
    cout << endl << match_cds(testDB, "el", true);
    cout << endl << number_of_cds(testDB);
	return 0;
}
