...
*/
    cin >> length.minutes >> length.seconds;
    return in;
}
...
{//             
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        {
            nr_artists ++;
            if(match (artist, tracks[i].artist) && display && tracks[i-1].artist != tracks[i].artist)
                {
                    show_track(tracks[i], td);
                    cout << '\n';
                }
...
{//             
    ...
    for (int i = 0; i < tracks.size(); i++)
    {
        ...
        {
            nr_cds ++;
            if(match (artist, tracks[i].artist) && display && tracks[i-1].cd != tracks[i].cd)
                {
                    show_track(tracks[i], td);
                    cout << '\n';
                }
...
*/
    ...
    ifstream music;
    int entries = 0;
    music.open( "Tracks.txt" );

    Track track_n;
    for (int i = 0 ; i < 6000 ; i++)
    {
        music >> track_n ;//                   
        show_track(track_n, lt) ;
        entries++;
    }
    cout << entries ;
