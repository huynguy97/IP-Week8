...
int match_tracks (vector<Track>& tracks, string track, bool display)
{
    ...
    /*               
                ...
                */
    int matches =0;
    TrackDisplay trackdisp = {true, true, true, true, true, true, true, true};

    for (int i = 0; i < tracks.size(); i++){
           if(match(track, tracks[i].title)) {
                matches++;
                if(display) show_track(tracks[i],trackdisp);
           }
    }
    return matches;
}
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    */
    ...
    TrackDisplay trackdisp = {true, false, false, false, false, false, false, false};

    vector<Track> Artistvec;

    for (int i = 0; i < tracks.size(); i++){
            if(match(artist, tracks[i].artist)) {
                    if (first_match){
                        first_match = false;
                        Artistvec.push_back(tracks[i]);
                    }
                    bool new_artist = true;
                    for(int j = 0; j < Artistvec.size() ; j++) if (Artistvec[j].artist==tracks[i].artist) new_artist=false;

                    if (new_artist) Artistvec.push_back(tracks[i]);
            }
    }

    if (display) for (int i=0; i < Artistvec.size(); i++) show_track(Artistvec[i], trackdisp);
...
int match_cd (vector<Track>& tracks, string artist, bool display)
{
    ...
    */
    bool first_match = true;
    TrackDisplay trackdisp = {true, true, true, false, false, false, false, false};
    ...
    for (int i = 0; i < tracks.size(); i++){
            if(match(artist, tracks[i].artist)) {
                    if (first_match){
                        first_match = false;
                        CDvec.push_back(tracks[i]);
                    }
                    bool new_cd = true;
                    for(int j = 0; j < CDvec.size() ; j++) if (CDvec[j].cd==tracks[i].cd) new_cd=false;

                    if (new_cd) CDvec.push_back(tracks[i]);
            }
    }

    if(display) for (int i=0; i < CDvec.size(); i++) show_track(CDvec[i], trackdisp);
...
*/
    ...
    cout << "tracks.txt is open: " << infile.is_open() << endl;

    if (infile.is_open()){

        while(infile){
            infile >> track;
            if(infile) Tracks.push_back(track);
        }
        infile.close();
    }
    ...
    cout << "The total number of tracks equals: " << Tracks.size() << endl;
