...
istream operator<< (ostream out, Track& track)
{
    out << track.artist << track.cd << track.year << track.track << track.title << track.tags << track.time << track.country;
}
...
void DisplayAll (TrackDisplay lt)
{
    lt.showArtist = true;
    lt.showAlbum = true;
    lt.showYear = true;
...
int match_tracks (vector<Track>& testDB, string track, bool display)
{
    string sub = track;
    string source;
    int amount = 0;
    ...
    if (display)
        DisplayAll(lt);
    for (int i = 4; i <= testDB.size(); i = i + 8)
    {
        source = testDB[i];
        if (match(sub, source))
        {
...
int match_artists (vector<Track>& tracks, string artist, bool display)
{
    ...
    cout << "Naar welke letters in een artiest wilt u zoeken?" << endl;
    string b;
    cin >> b;
    int teller;
    int i;
    ...
    {
        if(match(b, tracks[i].artists) && tracks[i].artist != f)
        {
