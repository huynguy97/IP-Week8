...
*/
    ...
    {
        cout << "Track title: " << track.title << endl;
    }
    if (lt.showTags == true)
    {
    ...
    {
        cout << "Country (artist): " << track.country << endl;
    }
    cout << endl;
}
...
int match_tracks(vector<Track>& tracks, string track, bool display)
{
    ...
    for (int j = 0; j < tracks.size(); j++)
        {
            if (match(track, tracks[j].title))
            {
                 noTracks++;
                 if (display == true)
                 {
                        show_track(tracks[j], {true,true,true,true,true,true,true,true});
                 }
            }
        }
...
int match_artists(vector<Track>& tracks, string artist, bool display)
{
    ...
    for (int j = 0; j < tracks.size(); j++)
        {
            if (match(artist, tracks[j].artist))
            {
                if (tracks[j].artist != lastArtist)
                {
                    noArtists++;
                    lastArtist = tracks[j].artist;
                    if (display == true)
                    {
                        show_track(tracks[j], td);
                    }
                }
            }
        }
    ...
    for (int j = 0; j < tracks.size(); j++)
        {
            if (match(artist, tracks[j].artist))
            {
                if (tracks[j].cd != lastCDTitle)
                {
                    noCDTitles++;
                    lastCDTitle = tracks[j].cd;
                    if (display == true)
                    {
                        show_track(tracks[j], td);
                    }
...
//                            
    ...
    if (input_file)
    {
        ...
        cout << "The number of entries read: " << noTracks << endl;

        return 0;
    }
    else
    {
        cout << "Tracks.txt could not be openend." << endl;
    }
