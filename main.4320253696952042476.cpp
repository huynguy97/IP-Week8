...
*/
    ...
    sum.minutes = sum.minutes + (a.seconds + b.seconds - sum.seconds)/60;
    return sum;
...
*/
    ...
    for(int i = 0; i < vector_size; i++)
    {
        ...
        {
            av_size = artist_vector.size();
            in_vector = false;
            for(int j = 0; j < av_size; j++)
            {
                if(tracks[i].artist == artist_vector[j])
                   in_vector = true;
            }
            if (!in_vector)
            {
                artist_vector.push_back(tracks[i].artist);
                if (display)
                    show_track(tracks[i], td);
            }
        }
...
*/
    ...
    for(int i = 0; i < vector_size; i++)
    {
        if(match(artist, tracks[i].artist))
        {
            cdv_size = cd_vector.size();
            in_vector = false;
            for(int j = 0; j < cdv_size; j++)
            {
                if(tracks[i].cd == cd_vector[j])
                    in_vector = true;
            }
            if(!in_vector)
            {
                cd_vector.push_back(tracks[i].cd);
                if(display)
                    show_track(tracks[i], td);
            }
        }
    }
    return cd_vector.size();
...
/*                
    ...
    */
    ifstream track_file ("Tracks.txt");
    vector<Track> tracks_vector;
    Track track;
    TrackDisplay td = {true, true, true, true, true, true, true, true};
    if(track_file)
    {
        while(track_file)
        {
            ...
            if(track_file)
            {
                tracks_vector.push_back(track);
                show_track(track, td);
            }
        }
        cout << "The number of enteries is " << tracks_vector.size();
    }
    else
    {
