...
vector<Track> testDB = {{"Adele",                          "19",                          2008,1, "Daydreamer",                  "pop,debut album",                   { 3,40},"England"}
                       ...
                       ,{"Adele",                          "21",                          2011,3, "Turning terings",             "pop",                               { 4,10},"England"}
...
int match_artists(vector<Track>& tracks, string artist, bool display)
{
    ...
    for(int i = 0; i < tracks.size(); i++)
    {
        ...
        {
            if (foundmatch != tracks[i].artist)
            {
                matches += 1;
                foundmatch = tracks[i].artist;
...
int match_cds(vector<Track>& tracks, string cd, bool display)
{
    ...
    for(int i = 0; i < tracks.size(); i++)
    {
        if(match(cd, tracks[i].cd))
        {
            if (foundmatch != tracks[i].cd)
            {
                matches += 1;
                foundmatch = tracks[i].cd;
