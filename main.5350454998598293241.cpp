...
int match_artists (vector<Track>& tracks, string artist, bool display) {
    ...
    int counter = 0;
    string former_artist = " ";
    for (int i = 0; i < v_size(tracks); i++) {
        if (match(artist, tracks[i].artist) && tracks[i].artist != former_artist) {
            counter = counter + 1;
            former_artist = tracks[i].artist;
            if (display) {
...
int match_cds (vector<Track>& tracks, string artist, bool display) {
    ...
    string former_cd = " ";
    for (int i = 0; i < v_size(tracks); i++) {
        if (match(artist, tracks[i].artist) && tracks[i].cd != former_cd) {
            counter = counter + 1;
            former_cd = tracks[i].cd;
            if (display) {
...
*/
    ...
    for (int j = 0; j < 5891; j++) {
        infile >> (input_track);
        input_DB.push_back(input_track);
        check_counter = check_counter + 1;
    }
    for (int k = 0; k < v_size(input_DB); k ++) {
        show_track(input_DB[k], {true, true, true, true, true, true, true, true});
    }
    cout << check_counter << " entries found" << endl;
	return 0;
}
