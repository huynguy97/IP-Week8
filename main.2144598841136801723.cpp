...
*/
    ...
    if(length.minutes < 10) out << "0" << length.minutes;
    else out << length.minutes;

    cout << ":";

    if(length.seconds < 10) out << "0" << length.seconds;
...
bool vector_contains(vector<string> vector, string artist)
{//              
	assert(true);
/*               
 ...
 */
	int size = tracks.size();
	int amount_of_artists = 0;
	vector<string> displayed_artists;
	...
	for(int i = 0; i < size; i++)
	{
		...
		if(match(artist, artists) && !vector_contains(displayed_artists, artists))
		{
			amount_of_artists++;
			displayed_artists.push_back(artists);
 ...
 */
	...
	for(int i = 0; i < size; i++)
	{
		...
		if(match(artist, artists) && !vector_contains(displayed_cds, cds))
		{
			amount_of_cds++;
			displayed_cds.push_back(cds);

			if(display)
			{
				TrackDisplay td = {false, true, false, false, false, false, false, false};
				show_track(t, td);
			}
		}
	}
	return amount_of_cds;
}
 ...
 */
	...
	for(int i = 0; i < size; i++)
	{
		...
		if(!vector_contains(counted_cds, cds))
		{
			amount_of_cds++;
			counted_cds.push_back(cds);
		}
	}
	return amount_of_cds;
}
...
bool open_input_file (ifstream& infile)
{
	...
	cout << "Please enter the name of the file (without spaces)" << endl;
	string input;
	cin >> input;
	infile = (ifstream) input.c_str();
...
void load_file_to_program(ifstream& infile)
{
	//              
...
*/
	ifstream infile;
	while(!open_input_file(infile));

	load_file_to_program(infile);
}
