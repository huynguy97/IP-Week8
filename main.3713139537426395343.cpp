...
*/
    Length sum;
    sum.minutes=(a.seconds+b.seconds)/60+a.minutes+b.minutes;
...
*/
    ...
    for (int i = 0; i < tracks.size(); ++i)
    {
        string curr=tracks[i].artist;
        if (match(artist, curr))
        {
            bool dub_found=false;
            int j=0;
            while(!dub_found && j<Found.size())
            {
                if(Found[j]==curr)
                {
                    dub_found=true;
...
*/
    ...
    for (int i = 0; i < tracks.size(); ++i)
    {
        ...
        if (match(artist, curr))
        {
            string cd_of_curr=tracks[i].cd;
            bool dub_found=false;
            int j=0;
            while(!dub_found && j<Found.size())
            {
                if(Found[j]==cd_of_curr)
                {
                    dub_found=true;
                }
                j++;
...
*/
    ...
    for (int i = 0; i < tracks.size(); ++i)
    {
        string album_name = tracks[i].cd;
        bool dub_found = false;
        int j = 0;
        while (!dub_found && j < Found.size())
        {
            if (Found[j] == album_name)
            {
                dub_found = true;
            }
            j++;
...
cout<<"\nThe total number of tracks imported is: "<<tracks.size()-1<<endl;

    cout<<"\nThe assignment queries:\n\n Now the matched tracks with el:"<<endl;
    match_tracks(testDB, "el", true);
    cout<<"\nNow the match artists with el:"<<endl;
    match_artists(testDB, "el", true);
    cout<<"\nNow the match cds with el:"<<endl;
    match_cds(testDB, "el", true);
    cout<<"\nNow the No of cds:"<<endl;
